...
@section('title', 'Renew Domain')
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card m-3">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h3 class="card-title">{{ __('Renew Domain') }}</h3>
                    <div class="button_">
                ...
                <div class="card-body">
                    <form action="{{ route('cm.ced.ced_renew', $domain->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="card-body">
                            <div class="row">
                                <div class="form-group col-md-6">
                                    <label>{{ __('Client') }}<span class="text-danger">*</span></label>
                                    <input type="text" value="{{ $domain->client->name }}" class="form-control" disabled>
                                </div>
                                <div class="form-group col-md-6">
                                    <label>{{ __('Hosting') }}<span class="text-danger">*</span></label>
                                    <input type="text" value="{{ $domain->hosting ? $domain->hosting->name : 'N/A' }}"
                                        class="form-control" disabled>
                                </div>
                                <div class="form-group col-md-6">
                                    <label>{{ __('Domain') }}<span class="text-danger">*</span></label>
                                    <input type="text" value="{{ $domain->domain_name }}" class="form-control" disabled>
                                </div>
                                <div class="form-group col-md-6">
                                    <label>{{ __('Renew From') }}<span class="text-danger">*</span></label>
                                    <input type="date" name="renew_from" class="form-control"
                                        value="{{ $domain->active_renew() ? $domain->active_renew()->expire_date : $domain->expire_date }}">
