                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="form-group col-md-6">
                                    <label for="clien_id">{{ __('Client') }}<span class="text-danger">*</span></label>
                                    <select name="clien_id" id="clien_id" class="form-control">
                                        <option selected hidden value=" ">{{ __('Select Client') }}</option>
                                        @foreach ($clients as $client)
                                            <option value="{{ $client->id }}"
                                                {{ old('clien_id') == $client->id ? 'selected' : '' }}>
                                                {{ $client->name }}</option>
                                        @endforeach
                                    </select>
                                    @include('alerts.feedback', ['field' => 'clien_id'])
                                </div>
...
@push('js')
    <script>
        ...
        $(document).ready(function() {
            $('#clien_id').on('change', function() {
                let client = $(this).val();
                activeRenewFor($(this).val());

            });
            activeRenewFor($('#clien_id').val());
        });

        $(document).ready(function() {
            $('#renew_for').on('change', function() {
                let renew_for = $(this).val();
                let client_id = $('#clien_id').val();
                let url = (
